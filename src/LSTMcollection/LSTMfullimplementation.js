...
function LSTMfullimplementation(){
    ...
    const codeString3 = 'class LSTMmainalgo:\n    def __init__(self, inputarry, outputarry, hidden_dim) -> None:\n        self.inputsize  = len(inputarry)\n        outputsizeArg = len(outputarry)\n        self.input = inputarry\n        self.output = outputarry\n        self.inputsize = self.inputsize\n        self.outputsize = outputsizeArg\n        self.hidden_dim = hidden_dim\n\n        #weight initialization\n        self.Wf = np.random.uniform( -1 / np.sqrt(self.inputsize), 1/  np.sqrt(self.inputsize), (self.hidden_dim,self.inputsize) )\n        self.Wi = np.random.uniform( -1/  np.sqrt(self.inputsize), 1/ np.sqrt(self.inputsize), (self.hidden_dim,self.inputsize)  )\n        self.Wo = np.random.uniform( -1/ np.sqrt(self.inputsize), 1/ np.sqrt(self.inputsize), (self.hidden_dim,self.inputsize)   )\n        self.Wc = np.random.uniform( -1/ np.sqrt(self.inputsize), 1/ np.sqrt(self.inputsize), (self.hidden_dim,self.inputsize)   )\n        self.Wv = np.random.uniform( -1/ np.sqrt(self.inputsize), 1/ np.sqrt(self.inputsize),(self.outputsize,self.hidden_dim)   )\n\n        #storage initialization\n        ct = np.zeros((self.hidden_dim,1))\n        self.ctstorage = [ct]\n        self.oinpstorage = []\n        self.ostorage = []\n        self.astorage = []\n        self.ctdstorage = []\n        self.itinputstorage = []\n        self.a1storage = []\n        self.finputstorage = []\n        self.ctdinpstorage = []\n        self.ftstorage = []\n        self.itstorage = []\n        self.bstorage = []'
