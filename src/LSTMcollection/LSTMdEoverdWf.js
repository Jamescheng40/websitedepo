...
function LSTMdEoverdWf(){
    ...
    const itwf = '$$ {i_t = }\\sigma_s{(\\sum_{i=0}^M W_i \\otimes x_t + bias(optional))} = \\sigma_s{( W_i \\odot x_t + bias(optional))} = \\sigma_s{(d3)} \\; {where \\; d3=\\sigma_s(\\sum_{i=0}^M W_i \\otimes x_t) \\; is \\; sigmoid \\; and \\; d6=\\sum_{i=0}^M d7}, \\; d7=W_i \\otimes x_t   $$';
    ...
    const dgoverdP_i_final = "$$ \\frac{\\partial E}{\\partial g_j} = diag(Y_i/P_i) \\ni T \\times T  $$"
    ...
    const dL2overdhfinalproduct = "$$ \\frac{\\partial L2_1}{\\partial h_t} = \\begin{bmatrix} W_{v11} & 0 & \\cdots & 0 \\\\ 0 & W_{v12}  & \\cdots & 0 \\\\ \\vdots & \\vdots & \\vdots & \\vdots \\\\ 0 & 0 & \\cdots & W_{v1M} \\end{bmatrix} = diag(W_{v1i}) \\ni T \\times M $$"
    const dL22overdh2finalproduct = "$$\\frac{\\partial L2_2}{\\partial h_t} =\\begin{bmatrix} W_{v21} & 0 & \\cdots & 0 \\\\ 0 & W_{v22}  & \\cdots & 0 \\\\ \\vdots & \\vdots & \\vdots & \\vdots \\\\ 0 & 0 & \\cdots & W_{v2M} \\end{bmatrix} = diag(W_{v2i}) \\ni T \\times M  $$"
    const dL21overdh2timesdL2overdhfinalproduct = "$$ \\frac{\\partial L1_1}{\\partial L2_1} \\odot \\frac{\\partial L2_1}{\\partial h_t} = \\begin{bmatrix} 1 & 1 & \\cdots & 1 \\end{bmatrix} \\odot \\begin{bmatrix} W_{v11} & 0 & \\cdots & 0 \\\\ 0 & W_{v12}  & \\cdots & 0 \\\\ \\vdots & \\vdots & \\vdots & \\vdots \\\\ 0 & 0 & \\cdots & W_{v1M} \\end{bmatrix} = \\begin{bmatrix} W_{v11} & W_{v12} & \\cdots & W_{v1M}  \\end{bmatrix}  $$"
    const dL22overdh2timesdL2overdhfinalproduct = "$$\\frac{\\partial L1_2}{\\partial L2_2} \\odot \\frac{\\partial L2_2}{\\partial h_t}  = \\begin{bmatrix} 1 & 1 & \\cdots & 1 \\end{bmatrix} \\odot \\begin{bmatrix} W_{v21} & 0 & \\cdots & 0 \\\\ 0 & W_{v22}  & \\cdots & 0 \\\\ \\vdots & \\vdots & \\vdots & \\vdots \\\\ 0 & 0 & \\cdots & W_{v2M} \\end{bmatrix} =  \\begin{bmatrix} W_{v21} & W_{v22} & \\cdots & W_{v2M}  \\end{bmatrix} $$"
    const dL2ioverdh2ifinalproduct = "$$ \\frac{\\partial Li}{\\partial h_t} = \\begin{bmatrix} W_{v11} & W_{v12} & \\cdots & W_{v1M} \\\\ W_{v21} & W_{v22} & \\cdots & W_{v2M} \\\\ \\vdots & \\vdots & \\vdots \\\\ W_{vT1} & W_{vT2} & \\cdots & W_{vTM} \\end{bmatrix} \\ni T \\times M $$"
